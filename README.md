...
**Note:** by default dynamic websites (where content is loaded by js) may be saved not correctly because `the program ` doesn't execute js, it only parses http responses for html and css files.
This module is an Open Source Software maintained by one developer in free time. If you want to thank the author of this module you can use [PayPal](https://paypal.me/scodes14).
...
Default options you can find in [website-scraper/lib/config/defaults.js](./website-scraper/lib/config/defaults.js).
...
* utils - scraper [utils](./website-scraper/lib/utils/index.js)
...
Parameters - object which includes:
* resource - [Resource](./website-scraper/lib/resource.js) object
...
Default plugins which generate filenames: [byType](./website-scraper/lib/plugins/generate-filenamy-by-type-plugin.js), [bySiteStructure](./website-scraper/lib/plugins/generate-filenamy-by-site-structure-plugin.js)
```javascript
...
##### getReference
Action getReference is called to retrieve reference to resource for parent resource. Can be used to customize reference to resource, for example, update missing resource (which was not loaded) with absolute url. By default reference is relative path from `parentResource` to `resource` (see [GetRelativePathReferencePlugin](./website-scraper/lib/plugins/get-relative-path-reference-plugin.js)).

Parameters - object which includes:
* resource - [Resource](./website-scraper/lib/resource.js) object
* parentResource - [Resource](./website-scraper/lib/resource.js) object
...
## result
Array of [Resource](./website-scraper/lib/resource.js) objects containing:
